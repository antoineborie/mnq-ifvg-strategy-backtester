...
def test_config(name, config, verbose=False):
    ...
    if not verbose:
        ...
        print(f"\n  {name}: {total_trades}t WR={overall_wr:.1f}% P&L={total_pnl:+.0f} "
              f"TPW={tpw:.1f} B60={below_60}/{total_q} CONS={consistency:.0f}% B50={below_50} MLS={max_ls}")
...
best_base = {
    'min_fvg_size': 3.0, 'max_fvg_age_m15': 8, 'rr_target': 1.2,
...
print("=" * 80)
print("ITERATION 6: REFINING AGE=8 + OR + 1TPD (B60=2, CONS=93%)")
print("=" * 80)

print("\n=== BASELINE DETAIL ===")
test_config("AGE8_OR_1TPD", best_base, verbose=True)

print("\n=== MICRO-VARIATIONS ===")
for est in ['10:00', '10:05', '10:10']:
    test_config(f"EST={est}", {**best_base, 'entry_start_time': est})

for body in [50, 55, 60, 65]:
    test_config(f"BODY={body}", {**best_base, 'min_displacement_body_pct': body})

for ret in [50, 55, 60, 65, 70]:
    test_config(f"RET={ret}", {**best_base, 'retracement_pct': ret})

for ds in [3.0, 3.5, 4.0]:
    test_config(f"DSIZE={ds}", {**best_base, 'min_displacement_size': ds})

for fvg in [3.0, 3.5, 4.0]:
    test_config(f"FVG={fvg}", {**best_base, 'min_fvg_size': fvg})

for mr in [18, 20, 25]:
    test_config(f"RISK={mr}", {**best_base, 'max_risk_pts': mr})

print("\n=== MOST PROMISING COMBOS ===")
results = []
for body in [55, 60, 65]:
    for ret in [50, 55, 60]:
        for fvg in [3.0, 3.5, 4.0]:
            for ds in [3.0, 3.5]:
                for mr in [20, 25]:
                    cfg = {**best_base, 'min_displacement_body_pct': body,
                           'retracement_pct': ret, 'min_fvg_size': fvg,
                           'min_displacement_size': ds, 'max_risk_pts': mr}
                    r = test_config(f"B{body}/R{ret}/F{fvg}/D{ds}/MR{mr}", cfg)
                    if r:
                        results.append((r, cfg))

results.sort(key=lambda x: (-x[0]['consistency'], x[0]['below_60'], -x[0]['wr']))
print(f"\n=== TOP 15 COMBOS ===")
for i, (r, _) in enumerate(results[:15]):
...
if results:
    best_r, best_cfg = results[0]
    print(f"\n=== BEST COMBO DETAIL ===")
    test_config(f"BEST: {best_r['name']}", best_cfg, verbose=True)
